...
def handle_cli_errors(verbose: bool = False) -> Callable[[F], F]:
    """Decorator que captura excecoes comuns de CLI e exibe mensagens formatadas.

    Args:
        verbose: Se True, exibe traceback completo para erros genericos.
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                error_console.print(
                    ...
                    "  Este modelo requer autorizacao. Para resolver:\n\n"
                ...
                error_console.print(f"[bold red]✘ GPU nao suportada:[/bold red] {e}")
                raise Exit(1) from None
            except shared_types.InsufficientVRAMError as e:
                error_console.print(f"[bold red]✘ VRAM insuficiente:[/bold red] {e}")
                raise Exit(1) from None
            except FileNotFoundError as e:
                error_console.print(f"[bold red]✘ Arquivo nao encontrado:[/bold red] {e}")
                raise Exit(1) from None
            except PermissionError as e:
                error_console.print(f"[bold red]✘ Erro de permissao:[/bold red] {e}")
                raise Exit(1) from None
