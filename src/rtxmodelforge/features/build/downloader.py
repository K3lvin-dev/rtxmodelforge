...
def download_weights(
    ...
    try:
        ...
        raise ValueError(f"Modelo não encontrado no HuggingFace: {model_id}")
...
def _prompt_params_billions() -> float:
    ...
    console.print("[yellow]Não foi possível detectar o número de parâmetros automaticamente.[/yellow]")
    while True:
        val = console.input("[bold cyan]Qual o tamanho do modelo em bilhões de parâmetros? (ex: 8.0): [/bold cyan]")
        try:
            return float(val)
        except ValueError:
