...
from rtxmodelforge.shared import gpu, config
...
def check_nvidia_driver() -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            "Driver NVIDIA", 
            passed, 
            f"Versão {gpu_info.driver_version} (Mínimo: 525.x)"
        )
...
def check_libopenmpi() -> CheckResult:
    ...
    try:
        subprocess.check_call(["dpkg", "-s", "libopenmpi-dev"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def check_gpu() -> CheckResult:
    ...
    gpu_info = gpu.detect_gpu()
    if not gpu_info:
        return CheckResult("GPU NVIDIA", False, "Nenhuma GPU NVIDIA detectada ou nvidia-smi falhou.")
...
def check_hf_token() -> CheckResult:
    ...
    return CheckResult("HuggingFace Token", False, "Não configurado. Use 'rtxforge login'", blocking=False)
